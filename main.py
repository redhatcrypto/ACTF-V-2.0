...
def fetch_data(timeframe, limit=None):
    try:
        ...
        data = exchange.fetch_ohlcv(config["symbol"], timeframe, limit=limit)
        if not data:
...
def monitor_skipped_trades(state, current_price, df):
    ...
    if "trade_skipped" in state and state["trade_skipped"]:
     ...
     # Enforce max_skipped_trades without price movement condition
    if len(state["last_price_history"]) == window:
...
def place_order(side, quantity, dry_run=False):
    try:
        ...
        order = exchange.create_market_order(config["symbol"], side, quantity)
        ...
        try:
            ...
            if not in_position:
                ...
                if side == "buy" and current_price >= entry_price * (1 + config["take_profit"] / 2):
                    ...
                    order = exchange.create_market_order(config["symbol"], "sell", partial_exit_quantity)
                    if order:
                    ...
                    # Secure partial profits
                    partial_exit_quantity = quantity / 2
                    order = exchange.create_market_order(config["symbol"], "buy", partial_exit_quantity)
                    if order:
